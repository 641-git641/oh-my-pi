...
- `artifact://<id>` resolves a session artifact as plain text. Selector-paginated reads stream from the backing file at any size, but unbounded `:raw` (and whole-artifact inline reads) are blocked above `50 KiB` (`MAX_ARTIFACT_RAW_INLINE_BYTES`) with a workflow notice pointing at bounded ranges (`artifact://<id>:1-3000`, `artifact://<id>:raw:1-3000`) and the backing file path. Protocol-level full-content resolution is hard-capped at 8 MiB (`MAX_INLINE_ARTIFACT_BYTES` in `packages/coding-agent/src/internal-urls/artifact-protocol.ts`); larger artifacts reject the whole-resource read with the same selector and backing-path hints. Path-only consumers (search/grep, bash URL expansion) skip content materialization and work on artifacts of any size.
