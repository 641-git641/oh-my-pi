...
- The `{"name": …, "content": …}` nesting makes each result self-describing: the binding to its call is by name, not only by position. (Qwen3 instead emits the bare content under a `user` turn and relies on ordering — [qwen3.md](qwen3.md) §Tool-result format.)
...
- **Result binding:** classic Hermes 2 Pro ties each result to its call by the `{"name": …, "content": …}` nesting under a `tool` turn; Qwen3 ties them by ordering under a `user` turn. Parsers that accept both families should not assume either alone.
...
- omp implementation on `main` @ `028c0a4` — every omp-specific claim above is cited `file:line` inline
