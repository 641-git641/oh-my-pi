...
- **Region-pinned workspace residency**: Enterprise ChatGPT workspaces can be pinned to a data-residency region, and reject any Codex request whose egress region differs — HTTP 401 `Workspace is not authorized in this region.` — unless the client declares the workspace residency itself. `createCodexHeaders` reads it from the OAuth access token (`getCodexResidency` in `packages/catalog/src/wire/codex.ts`, claim `chatgpt_data_residency` with `chatgpt_compute_residency` as fallback) and sends `x-openai-internal-codex-residency` on both SSE and WebSocket transports, plus `generate_image`'s Codex path. Accounts without the claim (personal ChatGPT, opaque proxy keys that are not JWTs) send no header, and a caller-supplied header of the same name is never overwritten.
