...
- **Thought parts & signature retention**: `isThinkingPart` identifies reasoning text when `part.thought === true`. Encrypted `part.thoughtSignature` fields are preserved across deltas using `retainThoughtSignature`. In `convertMessages`, thought signatures are retained only when message provider/model match the target (`msg.provider === model.provider && msg.model === model.id`) and pass `isValidThoughtSignature` (base64 check). Gemini 3 tool calls lacking a valid signature omit `thoughtSignature` entirely (Gemini 3 requires it only on the first function call of a turn); the `skip_thought_signature_validator` bypass sentinel is NOT emitted because Cloud Code Assist / Antigravity and Vertex AI reject it with `400 INVALID_ARGUMENT`, which permanently wedges any session whose history contains parallel tool calls (#9638).
