...
3. Prove one unique match anchored on the changed line, or use `«*`.
4. After an error, send the supplied corrected payload verbatim — nothing was applied; NEVER freestyle a new guess.
5. Edit FIRST, straight from text already in the conversation: matching is fuzzy and every failure shows the current lines. Read only what neither the conversation nor an error shows.
</critical>
